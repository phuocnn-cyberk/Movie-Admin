...
interface DeleteQuestDialogContextType {
  isOpen: boolean;
  onClose: () => void;
  onDelete: () => void;
}

const DeleteQuestDialogContext =
  React.createContext<DeleteQuestDialogContextType | null>(null);

const useDeleteQuestDialog = () => {
  const context = React.useContext(DeleteQuestDialogContext);
  if (!context) {
    throw new Error(
      "useDeleteQuestDialog must be used within DeleteQuestDialog.Root"
    );
  }
  return context;
};
...
const Root: React.FC<RootProps> = ({ children, isOpen, onClose, onDelete }) => {
  return (
    <DeleteQuestDialogContext.Provider value={{ isOpen, onClose, onDelete }}>
    ...
    </DeleteQuestDialogContext.Provider>
  );
};
  ...
  return (
    <div className="px-6 pt-4">
      <p className="text-sm font-normal leading-6 text-[#8A8A8A]">
        This will permanently delete the quest. It'll be removed from the
        campaign and no longer available to users.
        <br />
        Users who already completed it will keep their points.
        <br />
      ...
      <div className="rounded-lg border border-[#E9E9E9] p-4">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col gap-1">
            ...
            <span className="text-sm font-normal text-[#648DB9]">
              Staking of USHD to receive sUSHD
            </span>
          </div>
        </div>
      </div>
...
const Actions: React.FC = () => {
  const { onClose, onDelete } = useDeleteQuestDialog();
